 ...
 *
 * @param deviceZoomLevel zoom level the image shall be scaled for
 *
 * @return true if image is refreshed
 * @since 3.126
 */
public static Long win32_getHandle (Image image, Integer deviceZoomLevel) {
	if(deviceZoomLevel == null || image.isDisposed()) {
		return image.handle;
	}
	if(image.metadataMap.get(deviceZoomLevel) != null) {
		return image.metadataMap.get(deviceZoomLevel).handle;
	}
	...
	if (image.imageFileNameProvider != null) {
		...
		if (image.dataAtBaseZoom == null && image.memGC == null) {
			...
			image.dataAtBaseZoom = new ElementAtZoom<>(image.getImageData(image.currentDeviceZoom), image.currentDeviceZoom);
		}
...
long [] createGdipImage(Integer zoomLevel) {
	long handle = Image.win32_getHandle(this, zoomLevel);
	switch (type) {
		case SWT.BITMAP: {
			BITMAP bm = new BITMAP();
...
public long internal_new_GC (GCData data) {
	...
	*/
	...
	if (data != null) {
		...
		data.device = device;
		data.image = this;
		data.font = Font.win32_new(device.getSystemFont(), this.currentDeviceZoom);
	}
	return imageDC;
}
...
public static Image win32_new(Image image, int targetZoom) {
	if (targetZoom != image.currentDeviceZoom) {
		Image.win32_getHandle(image, targetZoom);
	}
	return image;
}
}