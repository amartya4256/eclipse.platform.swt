...
void drawItem (GC gc, long hTheme, RECT clipRect, boolean drawFocus) {
	...
	if (image != null) {
		...
		if (imageHeight > headerHeight) {
			gc.drawImage (image, DPIUtil.scaleDown(rect.left, getZoom()), DPIUtil.scaleDown(rect.top + headerHeight - imageHeight, getZoom()));
		} else {
			gc.drawImage (image, DPIUtil.scaleDown(rect.left, getZoom()), DPIUtil.scaleDown(rect.top + (headerHeight - imageHeight) / 2, getZoom()));
		}
