			...
			if (OS.SendMessage (handle, OS.LVM_SUBITEMHITTEST, 0, pinfo) >= 0) {
				...
				if (drawForeground) {
					...
					if (image != null) {
						...
						rect = DPIUtil.scaleDown(rect, getZoom());
						gc.drawImage (image, rect.x, rect.y, rect.width, rect.height, DPIUtil.scaleDown(x, getZoom()), DPIUtil.scaleDown(y, getZoom()), DPIUtil.scaleDown(size.x, getZoom()), DPIUtil.scaleDown(size.y, getZoom()));
