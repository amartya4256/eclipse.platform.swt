...
static long create32bitDIB (Image image, int zoomLevel) {
	long handle = Image.win32_getHandle(image, zoomLevel);
...
public Point map (Control from, Control to, Point point) {
	...
	return DPIUtil.scaleDown(mapInPixels(from, to, point), getZoomLevelForMapping(from, to));
}
...
public Point map (Control from, Control to, int x, int y) {
	...
	return DPIUtil.scaleDown(mapInPixels(from, to, x, y), getZoomLevelForMapping(from, to));
}
...
public Rectangle map (Control from, Control to, Rectangle rectangle) {
	...
	return DPIUtil.scaleDown(mapInPixels(from, to, rectangle), getZoomLevelForMapping(from, to));
}
...
public Rectangle map (Control from, Control to, int x, int y, int width, int height) {
	...
	return DPIUtil.scaleDown(mapInPixels(from, to, x, y, width, height), getZoomLevelForMapping(from, to));
}
