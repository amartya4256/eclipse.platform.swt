...
@Override Point computeSizeInPixels (int wHint, int hHint, boolean changed) {
	...
	if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
		if (itemCount > 0) {
			...
			long hTheme = 0;
			if (isAppThemed ()) {
				hTheme = display.hExplorerBarTheme (getCurrentDeviceZoom());
			}
...
void drawThemeBackground (long hDC, long hwnd, RECT rect) {
	...
	OS.DrawThemeBackground (display.hExplorerBarTheme (getCurrentDeviceZoom()), hDC, OS.EBP_NORMALGROUPBACKGROUND, 0, rect2, null);
}
...
public int getSpacing () {
	checkWidget ();
	return DPIUtil.autoScaleDown(getSpacingInPixels (), getCurrentDeviceZoom());
}
...
public void setSpacing (int spacing) {
	checkWidget ();
	setSpacingInPixels(DPIUtil.autoScaleUp(spacing, getCurrentDeviceZoom()));
}
