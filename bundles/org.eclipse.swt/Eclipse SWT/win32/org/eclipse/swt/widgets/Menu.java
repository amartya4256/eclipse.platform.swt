...
public void setLocation (int x, int y) {
	checkWidget ();
	setLocationInPixels(DPIUtil.autoScaleUp(x, getShell()), DPIUtil.autoScaleUp(y, getShell()));
}
...
void updateBackground () {
	...
	if (backgroundImage != null)
		hBrush = OS.CreatePatternBrush (backgroundImage.handleDPIChange(getCurrentDeviceZoom()));
	...
	if (wParam == ID_TOOLTIP_TIMER) {
		...
		if (selectedMenuItem != null && selectedMenuItem.parent != null) {
			...
			if (OS.PtInRect (rect, pt)) {
				...
				selectedMenuItem.showTooltip (pt.x, pt.y + getSystemMetrics2(OS.SM_CYCURSOR) / 2 + 5);
			} else {
				/*
