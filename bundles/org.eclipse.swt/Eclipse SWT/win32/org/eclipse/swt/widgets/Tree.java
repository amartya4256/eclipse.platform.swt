	...
	for (int i=0; i<Math.max (1, columnCount); i++) {
		...
		if (x + width > clientRect.left) {
					...
					if (explorerTheme) {
						if (OS.IsWindowEnabled (handle) && !hooks (SWT.EraseItem)) {
							...
							if (image != null) {
								...
								if (size == null) size = DPIUtil.scaleDown (getImageSize (), getZoom()); // To Points
								if (!ignoreDrawForeground) {
									GCData data = new GCData();
									data.device = display;
									GC gc = createNewGC(hDC, data);
									RECT iconRect = item.getBounds (index, false, true, false, false, true, hDC); // Pixels
									gc.setClipping (DPIUtil.scaleDown(new Rectangle(iconRect.left, iconRect.top, iconRect.right - iconRect.left, iconRect.bottom - iconRect.top), getZoom()));
									gc.drawImage (image, 0, 0, bounds.width, bounds.height, DPIUtil.scaleDown(iconRect.left, getZoom()), DPIUtil.scaleDown(iconRect.top, getZoom()), size.x, size.y);
			...
			if (drawItem) {
				...
				rect.left += DPIUtil.autoScaleUp(INSET - 1, getZoom());
				if (drawImage) {
					Image image = null;
					if (index == 0) {
						image = item.image;
					...
					int inset = i != 0 ? DPIUtil.autoScaleUp(INSET, getZoom()) : 0;
					int offset = i != 0 ? DPIUtil.autoScaleUp(INSET, getZoom()) : DPIUtil.autoScaleUp(INSET + 2, getZoom());
					if (image != null) {
						Rectangle bounds = image.getBounds (); // Points
						if (size == null) size = DPIUtil.scaleDown (getImageSize (), getZoom()); // To Points
						if (!ignoreDrawForeground) {
							//int y1 = rect.top + (index == 0 ? (getItemHeight () - size.y) / 2 : 0);
							int y1 = rect.top + DPIUtil.autoScaleUp((getItemHeight () - size.y) / 2, getZoom());
							int x1 = Math.max (rect.left, rect.left - inset + 1);
							GCData data = new GCData();
							data.device = display;
							GC gc = createNewGC(hDC, data);
							gc.setClipping (DPIUtil.scaleDown(new Rectangle(x1, rect.top, rect.right - x1, rect.bottom - rect.top), getZoom()));
							gc.drawImage (image, 0, 0, bounds.width, bounds.height, DPIUtil.scaleDown(x1, getZoom()), DPIUtil.scaleDown(y1, getZoom()), size.x, size.y);
							OS.SelectClipRgn (hDC, 0);
							gc.dispose ();
						}
						OS.SetRect (rect, rect.left + DPIUtil.autoScaleUp(size.x, getZoom()) + offset, rect.top, rect.right - inset, rect.bottom);
					} else {
						if (i == 0) {
				...
				case OS.CDDS_POSTPAINT: {
					...
					for (int i=0; i<columnCount; i++) {
						...
						if (columns[i].image != null) {
							...
							x += imageBounds.width + DPIUtil.autoScaleUp(12, getZoom());
							gc.drawImage (columns[i].image, DPIUtil.scaleDown(x, getZoom()), DPIUtil.scaleDown(y, getZoom()));
							gc.dispose ();
						}
						...
						if (drawForeground) {
							...
							if (image != null || index [0] == 0) {
								...
								if (image != null) {
									...
									gc.drawImage (image, rect.x, rect.y, rect.width, rect.height, DPIUtil.scaleDown(x, getZoom()), DPIUtil.scaleDown(imageRect.top, getZoom()), DPIUtil.scaleDown(size.x, getZoom()), DPIUtil.scaleDown(size.y, getZoom()));
