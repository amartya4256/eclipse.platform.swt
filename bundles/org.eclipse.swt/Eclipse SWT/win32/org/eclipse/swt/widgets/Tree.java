				...
				if (drawImage) {
					...
					if (image != null) {
						...
						if (!ignoreDrawForeground) {
							...
							int y1 = rect.top + DPIUtil.autoScaleUp((getItemHeight () - size.y) / 2, zoom);
						...
						OS.SetRect (rect, rect.left + DPIUtil.autoScaleUp(size.x, zoom) + offset, rect.top, rect.right - inset, rect.bottom);
					} else {
						if (i == 0) {
...
int imageIndex (Image image, int index) {
	...
	if (hwndHeader == 0 || getFirstColumnIndex() == index) {
		...
		*/
		long hImageList = ImageList.win32_getHandle(imageList, getZoom());
...
int imageIndexHeader (Image image) {
	...
	if (headerImageList == null) {
		...
		long hImageList = ImageList.win32_getHandle(headerImageList, getZoom());
		if (hwndHeader != 0) {
...
void updateImageList () {
	...
	* is to test for the same image list before setting the new one.
	*/
	long hImageList = i == items.length ? 0 : ImageList.win32_getHandle(imageList, getZoom());
	long hOldImageList = OS.SendMessage (handle, OS.TVM_GETIMAGELIST, OS.TVSIL_NORMAL, 0);
...
void updateOrientation () {
	...
	if (hwndHeader != 0) {
		if (headerImageList != null) {
			...
			long hImageListHeader = ImageList.win32_getHandle(headerImageList, getZoom());
				...
				case OS.CDDS_POSTPAINT: {
					...
					for (int i=0; i<columnCount; i++) {
						...
						if (columns[i].image != null) {
							...
							GC gc = createNewGC(nmcd.hdc, data);
							int zoom = getZoom();
							Rectangle imageBounds = DPIUtil.autoScaleBounds(columns[i].image.getBounds(), zoom, 100);
