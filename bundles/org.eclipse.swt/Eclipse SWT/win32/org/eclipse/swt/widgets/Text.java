...
private void addIcons() {
	if ((style & SWT.SEARCH) != 0) {
		if (!getShell().textSearchIcons.containsKey(getCurrentDeviceZoom())) {
			createSearchIcons();
		}
...
LRESULT WM_DRAWITEM (long wParam, long lParam) {
	...
	long hIcon = (struct.CtlID == SWT.ICON_SEARCH) ? getShell().textSearchIcons.get(getCurrentDeviceZoom()) : getShell().textCancelIcons.get(getCurrentDeviceZoom());
