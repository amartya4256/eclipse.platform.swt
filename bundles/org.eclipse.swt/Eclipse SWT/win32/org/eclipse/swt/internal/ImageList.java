...
public class ImageList {
	...
	HashMap<Integer, Long> zoomToHandle = new HashMap<>();
...
public static long win32_getHandle(ImageList imageList, int zoom) {
	if(!imageList.zoomToHandle.containsKey(zoom)) {
		int scaledWidth = DPIUtil.autoScaleUp(imageList.width, zoom);
		int scaledHeight = DPIUtil.autoScaleUp(imageList.height, zoom);
		long handle = OS.ImageList_Create (scaledWidth, scaledHeight, OS.ILC_MASK | OS.ILC_COLOR32, 16, 16);
		int count = OS.ImageList_GetImageCount (handle);
		for(int i = 0; i < imageList.images.length; i++) {
			Image image = imageList.images[i];
			if(image != null) {
				imageList.set (i, image, count, handle, zoom);
				count++;
			}
		}
		imageList.zoomToHandle.put(zoom, handle);
	}
	return imageList.zoomToHandle.get(zoom);
}
